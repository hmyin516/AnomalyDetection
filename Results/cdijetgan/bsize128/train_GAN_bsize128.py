...
# Training
BATCH_SIZE = 128 # 128 in paper, 32 in GitLab
...
# Plotting
PREFIX = "img-bsize128/{:.0f}SB-with".format(SB_WIDTH)
if not TAU21:
    PREFIX += "out"
PREFIX += "_tau21-with"
if not TAU32:
...
def train(dataset, testset, epochs, pretrain = False):
    for epoch in tqdm(range(epochs)):
        ...
        if save_model:
            generator.save("./models-bsize128/epoch{}-generator.h5".format(epoch + 1))
            discriminator.save("./models-bsize128/epoch{}-discriminator.h5".format(epoch + 1))
